...
public class AspenGroveItems {
    ...
    public static Item ASPEN_CHEST_BOAT;
    public static Item BOUQUET;

    public static void initialize() {
        ASPEN_BOAT = registerBoat("aspen_boat", AspenGroveBoatType.ASPEN, false);
        ASPEN_CHEST_BOAT = registerBoat("aspen_chest_boat", AspenGroveBoatType.ASPEN, true);
        BOUQUET = register("bouquet", new Item(new Item.Properties().stacksTo(16)));
    }

    private static Item registerBoat(String name, AspenGroveBoatType type, boolean chest) {
        ...
        Item item = new BoatItem(chest, Boat.Type.OAK, new Item.Properties().stacksTo(1)) {
            ...
            public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
                ...
                if (hitResult.getType() == HitResult.Type.BLOCK) {
                    Boat boat = chest
                            ? new AspenGroveChestBoatEntity(AspenGroveEntities.ASPEN_CHEST_BOAT, level)
                            : new AspenGroveBoatEntity(AspenGroveEntities.ASPEN_BOAT, level);

                    boat.setPos(hitResult.getLocation().x, hitResult.getLocation().y, hitResult.getLocation().z);
                    boat.setYRot(player.getYRot());

                    if (boat instanceof AspenGroveBoatEntity mb) mb.setModVariant(type);
                    if (boat instanceof AspenGroveChestBoatEntity mcb) mcb.setModVariant(type);
